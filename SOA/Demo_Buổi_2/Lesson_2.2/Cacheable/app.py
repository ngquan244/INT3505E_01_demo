...
def get_books():
    """
    Phản hồi được đánh dấu Cache-Control → client/browser có thể cache
    """
    resp = make_response(jsonify(BOOKS))
    resp.headers["Cache-Control"] = "public, max-age=30"  # cache trong 30s
    resp.headers["X-Server-Time"] = time.strftime("%H:%M:%S")  # để dễ quan sát
    return resp
